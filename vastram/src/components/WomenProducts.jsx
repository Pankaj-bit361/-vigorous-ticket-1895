...
import { getwomenproducts } from "../redux/productReducer/action"
...
console.log(data)
dispatch(getwomenproducts(ob))
},[location.search,page,sort,search])
...
<Dictaphone setsearch={setsearch}/>
  ...
  <Text marginTop={"0.8%"}>Women's Clothing</Text>
  <Flex gap={"5%"}>
  <Text mt={"4%"}>Sort</Text>
...
{data?.map((item)=>(
    ...
    <Link to={`/womens/${item.id}`}><Button mb={"2%"} colorScheme={"orange"}>View Details</Button></Link>
    </Box>
))}
</div>:<div >
{data?.map((item)=>(
    ...
    <Box> <Image  height={"250px"} width={"250px"} src={item.images.image1}></Image></Box>
   <Box w={"30%"} m={"auto"}>
